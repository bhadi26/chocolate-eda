...
As a chocolate lover, when I saw this chocolate related dataset on Kaggle, I couldn't resist the temptation to play around and see what I could learn. This notebook will walkthrough my typical steps of exploring a new data set.  For reference, the data I'm working with can be found at https://www.kaggle.com/rtatman/chocolate-bar-ratings/data. 
...
This isn't the most intitutive coefficient to interpret because we can't have a cocoa percent of zero (maybe that's possible somewhere, but not in this data set.) Let's try it again and rescale the cocoa percent to centered around the mean. 
...
For the mean cocoa percentage `r round(mean(chocolate$Cocoa.Percent.Numeric),2)`$\%$, we predict the rating to be 3.19. The coefficient of the cenetered cocoa rating is statistically significant. For every 1 standard deviation difference in cocoa rating (which is `r sd(chocolate$Cocoa.Percent.Numeric)`), we predict the rating to decrease -0.08.  This is not a large effect size and we see that the $r^2$ is very low at 0.027, indicating that the model does not do a great job of explaining the variation in our data set. 
